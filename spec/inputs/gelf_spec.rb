...
describe LogStash::Inputs::Gelf do
  ...
  context "when interrupting the plugin with TCP" do
    ...
    let(:chunksize) { 1420 }
    let(:protocol) { "TCP" }
    let(:producer) { InfiniteGelfProducer.new(host, port, chunksize, {:protocol => 1}) }
    let(:config) {  { "host" => host, "port" => port, "protocol" => protocol } }
  ...
  it "should be valid for protocol TCP" do
    ...
    chunksize = 1420
    protocol = "TCP"
    gelfclient = GELF::Notifier.new(host, port, chunksize, {:protocol => 1 })
